...
def validate_manifest(manifest: TaskManifest, *, require_frozen: bool = False) -> list[str]:
    ...
    if manifest.manifest_type not in {"pilot", "acquisition", "acquisition-extension", "evaluation", "evaluation-amended"}: errors.append("invalid manifest type")
    expected = {"pilot": "valid_seen", "acquisition": "train", "acquisition-extension": "train", "evaluation": "valid_unseen", "evaluation-amended": "valid_unseen"}.get(manifest.manifest_type)
