...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("environment", "protocol", "acquisition-environment", "acquisition-protocol", "acquisition-extension-environment", "acquisition-extension-protocol",
                 "evaluation-amendment", "evaluation-environment", "evaluation-protocol"):
    ...
    item = amended_sub.add_parser("check", help="NON-SCIENTIFIC end-to-end evaluation check on valid_seen with the real model.")
    item.add_argument("--run-id", required=True); item.add_argument("--max-runs", type=int); item.add_argument("--resume", action="store_true")
    item = amended_sub.add_parser("check-report"); item.add_argument("--run-id", required=True)
    item = amended_sub.add_parser("prepare-approvals", help="Write UNAPPROVED amendment, task, environment, and protocol requests; never approves.")
    item.add_argument("--preparation"); item.add_argument("--evidence-report", required=True)
    item = amended_sub.add_parser("freeze-tasks", help="Freeze the human-approved evaluation tasks with their controlled failures.")
    item.add_argument("--proposal", required=True); item.add_argument("--controlled-failures", required=True); item.add_argument("--approval-file", required=True); item.add_argument("--yes", action="store_true")
    item = amended_sub.add_parser("build-libraries", help="Build the Rule A libraries from the completed human core review (after amendment approval).")
    item.add_argument("--review-file"); item.add_argument("--yes", action="store_true")
    amended_sub.add_parser("plan", help="Evaluation launch gate (dry run).")
    item = amended_sub.add_parser("preflight", help="Every technical evaluation check; starts no episode.")
    item.add_argument("--backup-dir"); item.add_argument("--preparation")
    for name in ("run", "resume", "retry-failed"):
        item = amended_sub.add_parser(name); item.add_argument("--shard", type=int, required=True, choices=range(1, 7))
        item.add_argument("--yes", action="store_true"); item.add_argument("--backup-dir"); item.add_argument("--require-backup", action="store_true"); item.add_argument("--max-runs", type=int)
    item = amended_sub.add_parser("validate"); item.add_argument("--shard", type=int, choices=range(1, 7))
    item = amended_sub.add_parser("merge"); item.add_argument("--yes", action="store_true")
    item = amended_sub.add_parser("rater-export"); item.add_argument("--yes", action="store_true")
    item = amended_sub.add_parser("analyze"); item.add_argument("--rater-a"); item.add_argument("--rater-b"); item.add_argument("--adjudicated")
